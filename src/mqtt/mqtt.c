...
// Switch broker after this many consecutive disconnects without a successful connect
...
{
    return s_active_broker == 0 ? config_get_mqtt_broker1() : config_get_mqtt_broker2();
}
    ...
    switch ((esp_mqtt_event_id_t)event_id) {
        ...
        case MQTT_EVENT_DISCONNECTED:
            ...
            if (!s_ever_connected) {
                ...
                s_fail_count++;
                ESP_LOGW(TAG, "broker%d failed to connect (%d/%d)",
                         s_active_broker + 1, s_fail_count, BROKER_FAIL_THRESHOLD);
        ...
        case MQTT_EVENT_ERROR:
            if (event->error_handle->error_type == MQTT_ERROR_TYPE_TCP_TRANSPORT) {
                ...
                ESP_LOGW(TAG, "connection refused: 0x%x", event->error_handle->connect_return_code);
            }
            break;
...
void mqtt_publish(const char *topic, const char *data)
{
    if (mqtt_client_handle && (xEventGroupGetBits(mqtt_event_group) & MQTT_CONNECTED_BIT)) {
        int msg_id = esp_mqtt_client_publish(mqtt_client_handle, topic, data, 0, 1, 0);
